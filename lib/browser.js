...
async function connectBrowser() {
  ...
  if (useSparticuz) {
    const { prepareServerlessChromium } = require('./vercel-chromium');
    const chromiumPkg = require('@sparticuz/chromium');
    const { chromium: pwChromium } = require('playwright-core');

    const { executablePath, launchEnv } = await prepareServerlessChromium(chromiumPkg);
...
/** alert/confirm 등 — 미처리 시 CDP에서 ProtocolError로 프로세스가 종료될 수 있음 */
...
module.exports = { connectBrowser, isCdpAvailable, DEFAULT_CDP_URL, attachPageGuards };
