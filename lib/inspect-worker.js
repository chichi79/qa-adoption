...
function buildInspectOptions(body) {
  const includeMobile = body.mobile !== false;
  const viewports = normalizeViewports(
    includeMobile ? body.viewports || ['desktop', 'mobile'] : ['desktop'],
  );
  const maxLinksCap = isVercel ? (includeMobile ? 3 : 5) : 30;
  return {
    maxLinks: Math.min(Number(body.maxLinks) || 15, maxLinksCap),
    viewports,
